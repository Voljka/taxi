...
function DailyCtrl($scope, $state, data, TripService, PayoutService, DebtService, Flash) {
   ...
   $scope.selectDriver = function(for_date, driver){
      ...
      if (! driver.selected) {
         ...
         if (daysBetween($scope.lastReport, for_date).toFixed(0) < 2 && driver.uber_completeness && driver.gett_completeness ) {
   ...
   function recalcWage(driver){
      ...
      driver.left_to_pay = (driver.wage - driver.total_payouts + driver.covered + driver.income_deficit + driver.deferred_debt - driver.total_cash + driver.fuel)
   }
  ...
  $scope.checkPayoutEnter = function(keyEvent, payout) {
   ...
   $scope.saveDriverCalc = function(){
      ...
      if (deficitAfterAdjustments < 0) {
         ...
         // if (d.driver_part == 0.6)
         //    data.wage_rule = 2;
         // if (d.driver_part == 0.5)
         //    data.wage_rule = 3;
         // if (d.driver_part == 0.4)
         //    data.wage_rule = 4;
